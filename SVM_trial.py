...
def lemmatize_and_remove_stopwords(entry):
    #for entry in enumerate(df['text']):
    ...
    #    #OPTIONAL: Change bias words to same country name (example: "Canada")
...
tr_data_df["Tweet_Text"] = process_map(parrallel_tokenize, [entry for entry in tr_data_df["Tweet_Text"].to_list()], max_workers=4) 
x = process_map(lemmatize_and_remove_stopwords, [entry for entry in tr_data_df["Tweet_Text"].to_list()], max_workers=4, chunksize=1)
...
for i in range(0,10):    
    ...
    # #OPTIONAL: Prints the words in the feature vector (only executed on first loop) 
    # if i == 0:
    #     print(Tfidf_vect.vocabulary_, '\n')

    # #OPTIONAL: Prints the word vector (only executed on first loop) 
    # if i == 0:
    #     print(Train_X_Tfidf, '\n')
    ...
    # print(classification_report(Test_Y, predictions_NB,target_names=['yes','no']))
    ...
    # print(classification_report(Test_Y, predictions_SVM,target_names=['yes','no']))
    # #OPTIONAL: prints the confusion matrix associated with the SVM predictions
